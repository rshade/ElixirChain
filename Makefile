...
help:
	...
	@echo "  make quality     - Run all checks (format, lint, dialyzer, security, docs)"
...
test-watch:
	...
	@MIX_ENV=test MIX_REBAR3=$(REBAR3) $(MISE_EXEC) mix test.watch
...
test-file:
	@if [ -z "$(FILE)" ]; then \
		...
		MIX_ENV=test MIX_REBAR3=$(REBAR3) $(MISE_EXEC) mix test $(FILE); \
	fi
...
# Print discovered REBAR3 path (used by docs)
print-REBAR3:
	@echo $(REBAR3)
